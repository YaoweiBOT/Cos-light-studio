...
export async function buildApp(){
  ...
  for(const name of ['three','three-mesh-bvh','three-gpu-pathtracer','xatlas-web']){
  ...
  await writeFile(resolve(DIST,'build-manifest.json'),JSON.stringify({application:'cos-light-studio',version:'0.3.0',sourceDigest:await sourceDigest(),bundled:true,externalImports:externals.length,hasScannedHead:await exists(resolve(DIST,'models/LeePerrySmith/LeePerrySmith.glb')),outputs:Object.keys(result.metafile.outputs)},null,2)+'\n');
