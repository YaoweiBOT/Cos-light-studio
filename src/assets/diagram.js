...
export class LightingDiagram {
  ...
  draw(){
    ...
    const r=s.room;if(r.board!=='off'){
      c.save();const p=r.board==='below'?this.at(0,.35):this.at(r.boardSide*r.boardDistance,.1);c.translate(p.x,p.y);c.rotate(r.board==='below'?Math.PI/2:-r.boardSide*r.boardAngle*DEG);c.fillStyle=r.board==='black'?'#555a61':r.board==='silver'?'#bdcbd7':'#d3cfc2';c.fillRect(-3,-.4*scale,6,.8*scale);c.restore();
    }
    effectiveLights(s).forEach(l=>{
    ...
    c.save();c.translate(cx,cy);c.rotate(-s.model.yaw*DEG);c.fillStyle='#9ba29f';c.strokeStyle='#c0c5bf';c.lineWidth=1;
    ...
    const current=s.lights.find(l=>l.id===s.selected),a=angularSize(current);c.fillStyle='#7d8895';c.textAlign='left';c.font='12px system-ui';c.fillText(`灯心高 ${current.heightY.toFixed(2)} m`,18,h-38);c.fillText(`至面心 ${a.distance.toFixed(2)} m`,18,h-20);
    c.restore();
  }
}
