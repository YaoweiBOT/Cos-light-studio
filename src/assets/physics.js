...
export function aim(light) { const target=light.targetPoint||FACE;return { x: target.x+light.aimX, y: target.y + light.aimY, z: target.z+light.aimZ }; }
...
export function angularSize(light,point=light.targetPoint||FACE) {
  const d=length(sub(position(light),point)),w=light.type==='fresnel'?light.aperture:light.width,h=light.type==='fresnel'?light.aperture:light.height;
  return { width:2*Math.atan(w/(2*d))/DEG, height:2*Math.atan(h/(2*d))/DEG, distance:d };
}
export function spotProfile(light,cosine){
  const angle=light.beam*DEG/2,outer=Math.cos(angle),inner=Math.cos(angle*(1-light.edge));
  const t=clamp((cosine-outer)/Math.max(1e-9,inner-outer),0,1);return t*t*(3-2*t);
}
// Exact angular integral of smoothstep in cosine space: outer transition has mean 1/2.
// This keeps the ideal beam's far-field integrated flux fixed when focusing.
export function spotSolidAngle(light){const a=light.beam*DEG/2;return 2*Math.PI*(1-(Math.cos(a)+Math.cos(a*(1-light.edge)))/2);}
export const spotIntensity=light=>flux(light)/Math.max(1e-8,spotSolidAngle(light));
export const beamDiameter=(light,distance)=>light.aperture+2*distance*Math.tan(light.beam*DEG/2);
export function spotMeter(light,point=light.targetPoint||FACE,normal={x:0,y:0,z:1},steps=20){
  const {p,n,x,y}=emitterBasis(light);
  let sum=0,count=0;
  for(let j=0;j<steps;j++)for(let i=0;i<steps;i++){
    const u=(i+.5)/steps-.5,v=(j+.5)/steps-.5;if(u*u+v*v>.25)continue;
    const offset=add(mul(x,u*light.aperture),mul(y,v*light.aperture));
    const q=add(p,offset),delta=sub(q,point),r2=dot(delta,delta),l=unit(delta);
    const beamRay=unit(sub(point,q));
    sum+=Math.max(0,dot(normal,l))*spotProfile(light,dot(beamRay,n))/Math.max(r2,1e-8);count++;
  }
  return spotIntensity(light)*sum/Math.max(1,count);
}
...
export function meter(light, point=light.targetPoint||FACE, normal={x:0,y:0,z:1}, steps=20) {
  if (!light.enabled) return 0;
  if(light.type==='fresnel')return spotMeter(light,point,normal,steps);
  const {p,n,x,y}=emitterBasis(light);
  let sum=0;
  for(let j=0;j<steps;j++) for(let i=0;i<steps;i++) {
