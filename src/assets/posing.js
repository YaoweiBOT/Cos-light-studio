// Original pose rig, metres. All three female designs depict adults.
export const CHARACTERS=[['cute','杏 · 可爱圆脸'],['fresh','澪 · 清爽少女风（成年）'],['elegant','凛 · 成熟御姐'],['scan','Lee · 男性扫描']];
export const HAIRSTYLES=[['none','无假发'],['bob','齐刘海短波波'],['long','姬发式长直发'],['twin','高双马尾'],['ponytail','高单马尾'],['buns','双丸子头'],['side','偏分侧马尾'],['short','蓬松短碎发']];
export const HAIR_COLORS=[['ink','墨黑'],['silver','银白'],['pink','樱粉'],['blue','冰蓝'],['violet','紫罗兰'],['blonde','浅金'],['red','酒红']];
export const POSES=[['relaxed','自然站立'],['hip','单手叉腰'],['wave','抬手打招呼'],['frame','双手靠近脸'],['walk','前后错步'],['sit','坐三步梯'],['stool','坐圆凳']];
...
export function posePreset(id='relaxed'){
...
export const facePoint=model=>rigPose(model).joints.head;
export function setPose(state,id){
  state.model.object='head';state.model.body=true;state.model.pose=posePreset(id);
...
export function frameSubject(state,framing){
  ...
  Object.assign(state.camera,framing==='head'?{distance:1.65,focal:56,heightY:y,targetY:y-.025}:framing==='half'?{distance:2.1,focal:45,heightY:y-.15,targetY:y-.32}:{distance:3.5,focal:40,heightY:1.05,targetY:y*.55});
